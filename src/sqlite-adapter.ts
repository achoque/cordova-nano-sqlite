...
export class SQLiteStore implements NanoSQLStorageAdapter {
    ...
    public connect(complete: () => void) {
        ...
        fastALL(Object.keys(this._pkKey), (table, i, nextKey) => {
            this._sql(true, `CREATE TABLE IF NOT EXISTS ${table} (id TEXT PRIMARY KEY UNIQUE, data TEXT)`, [], () => {
    ...
    public batchRead(table: string, pks: any[], callback: (rows: any[]) => void) {
        let totalPks = pks.length;
        let startIndex = 0;
        const SQLITE_MAX_VARIABLE_NUMBER = 999;

        let rows: any[] = [];
        let totalBatches = 0, completed = 0;
        while (startIndex < totalPks) {
            let ids = pks.slice(startIndex, startIndex + SQLITE_MAX_VARIABLE_NUMBER);
            startIndex += SQLITE_MAX_VARIABLE_NUMBER;
            totalBatches += 1;

            this._sql(false, `SELECT data from ${this._chkTable(table)} WHERE id IN (${ids.map(p => "?").join(", ")}) ORDER BY id`, ids, (result) => {
