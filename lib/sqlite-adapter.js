...
var SQLiteStore = (function () {
    ...
    SQLiteStore.prototype.connect = function (complete) {
        ...
        console.log("NanoSQL \"" + this._id + "\" using SQLite3.");
        ...
        utilities_1.fastALL(Object.keys(this._pkKey), function (table, i, nextKey) {
            _this._sql(true, "CREATE TABLE IF NOT EXISTS " + table + " (id TEXT PRIMARY KEY UNIQUE, data TEXT)", [], function () {
    ...
    SQLiteStore.prototype.batchRead = function (table, pks, callback) {
        var totalPks = pks.length;
        var startIndex = 0;
        var SQLITE_MAX_VARIABLE_NUMBER = 999;
        var rows = [];
        var totalBatches = 0, completed = 0;
        while (startIndex < totalPks) {
            var ids = pks.slice(startIndex, startIndex + SQLITE_MAX_VARIABLE_NUMBER);
            startIndex += SQLITE_MAX_VARIABLE_NUMBER;
            totalBatches += 1;
            this._sql(false, "SELECT data from " + this._chkTable(table) + " WHERE id IN (" + ids.map(function (p) { return "?"; }).join(", ") + ") ORDER BY id", ids, function (result) {
                var i = result.rows.length;
                while (i--) {
                    rows.unshift(JSON.parse(result.rows.item(i).data));
                }
                completed += 1;
                if (totalBatches == completed) {
                    callback(rows);
                }
            });
        }
